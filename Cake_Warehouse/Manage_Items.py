...
def manage_cakes():
    ...
    if cakes:
        ...
        if not cake_row:
            ...
            # Editable Yield
            new_yield = st.number_input('Edit Percent Yield (%)', value=current_yield or 0.0, min_value=0.0,
                                        step=0.01, format='%.2f')
            ...
            c.execute('''
                      ...
                      FROM cake_ingredients ci
                               LEFT JOIN ingredients i
                                         ON ci.ingredient_or_subrecipe_id = i.id AND ci.is_subrecipe = 0
                               LEFT JOIN sub_recipes sr
                                         ON ci.ingredient_or_subrecipe_id = sr.id AND ci.is_subrecipe = 1
                      WHERE ci.cake_id = ?
                      ''', (cake_id,))
            ingredients = c.fetchall()
            ...
            for item_id, is_subrecipe, qty, item_name, item_type, ref_id in ingredients:
                new_qty = st.number_input(f"{item_name} ({item_type})", value=float(qty), step=0.00001,
                                          format="%.5f", key=f"{item_id}_qty_cake")
                ...
                if is_subrecipe:
                    ...
                    if total_weight:
                        ...
                        st.markdown(f"<small style='color:#888;'>[Formula] {formula_str}</small>",
            ...
            item_qty = st.number_input('Quantity (kg, L, etc)', min_value=0.0, step=0.00001, format="%.5f",
                                       key='item_qty')
            if st.button('Add to Cake'):
                try:
                    c.execute(
                        'INSERT INTO cake_ingredients (cake_id, ingredient_or_subrecipe_id, is_subrecipe, quantity) VALUES (?, ?, ?, ?)',
            ...
            cake_weight = st.number_input('Enter Total Cake Weight (kg)', min_value=0.001, step=0.001,
                                          format='%.3f')
            if cake_weight:
